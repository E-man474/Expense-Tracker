...
function Goals() {
  ...
  const handleAddSavings = async (goalId) => {
    ...
    if (isCompleted) alert(`🎉 Congratulations! "${goal.title}" goal is complete!`);
  };
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
          <div className="mb-5">
            ...
            <p className="text-sm text-gray-400 mt-1">Track your financial goals.</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center justify-center h-60 text-center">
              ...
              <p className="text-sm text-gray-400 mt-2 mb-4">Set your first savings goal.</p>
              <button onClick={() => setShowModal(true)}
                className="bg-green-500 text-white px-5 py-2.5 rounded-xl text-sm font-semibold">
                + Add Goal
              </button>
            </div>
          ) : (
                      ...
                      return (
                        <div key={item.id} className={`bg-white rounded-2xl p-5 border shadow-sm transition ${isPaused ? "border-yellow-200 opacity-75" : "border-gray-100"}`}>
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                {isPaused && (
                                  <span className="text-xs bg-yellow-100 text-yellow-600 px-2 py-0.5 rounded-full">Paused</span>
                                )}
                              </div>
                            </div>
                            <button
                          ...
                          <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden mt-4">
                            <div className={`h-full rounded-full transition-all ${progressColor(progress)}`}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-4">
              ...
              <input type="number" placeholder="Already Saved Amount (0 if none)"
